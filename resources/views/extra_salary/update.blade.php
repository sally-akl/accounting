													 ...
													 <div class="col-lg-12">
															 <div class="card">
																	 <div class="card col-lg-12 padding20">
																			 <div class="row">
																					 <div class=" mg-top25">
																							 <label class=" form-control-label"> <i class="far fa-edit"></i> @lang('app.update_extra_salary')</label>
																					 </div>
																			 </div>
																			 <div class="row">
																					 <div class="col-lg-12 mg-top30">
																						   @include("utility.error_messages")
																							 <form method="POST" action='{{url("extrasalary/{$extra_salary->id}")}}/{{app()->getLocale()}}?branch={{ Request::query("branch") }}'>
																                   @csrf
																								...
																								<input type="hidden" name="m_emp" value="{{ $extra_salary->emp_major_id}}" />
																								 ...
																								 <div class="col-sm-9">
																										 <select name="sal_min_extra" class="form-control" >
																											 ...
																											 @foreach ($salary_settings as $key => $sett)
																																				 <option value="{{$sett->id}}"  {{$sett->id == $extra_salary->extra_minus_id?"selected":"" }}  >	{{$sett->title}} ( {{$sett->percentage}}% ) </option>
																																			 @endforeach
																									 ...
																									 <button type="submit" class="btn btn-primary">{{ __('app.update_extra_salary') }} </button>
																							 </form>
																					 </div>
																			 </div>
